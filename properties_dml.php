...
function properties_insert(){
	...
	if($_REQUEST['SelectedID']){
		$res = sql("select * from properties where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
	...
	sql("insert ignore into membership_userrecords set tableName='properties', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function properties_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	<script>
		...
		<?php }else{ ?>
			...
			$j.ajax({
				...
				$j('[id=owner-container__RAND__], [id=owner-container-readonly__RAND__]').html('<span id="owner-match-text">' + resp.results[0].text + '</span>');
				if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=rental_owners_view_parent]').hide(); }else{ $j('.btn[id=rental_owners_view_parent]').show(); }
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(property_name)%%>', html_attr($row['property_name']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(number_of_units)%%>', html_attr($row['number_of_units']), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(owner)%%>', html_attr($row['owner']), $templateCode);
