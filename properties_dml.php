...
function properties_insert(){
	...
	$data['property_name'] = makeSafe($_REQUEST['property_name']);
		if($data['property_name'] == empty_lookup_value){ $data['property_name'] = ''; }
	$data['type'] = makeSafe($_REQUEST['type']);
		if($data['type'] == empty_lookup_value){ $data['type'] = ''; }
	$data['number_of_units'] = makeSafe($_REQUEST['number_of_units']);
	...
	if($_REQUEST['SelectedID']){
		$res = sql("select * from properties where id='" . makeSafe($_REQUEST['SelectedID']) . "'");
	...
	if(function_exists('properties_before_insert')){
		$args=array();
		if(!properties_before_insert($data, getMemberInfo(), $args)){ return false; }
	}
	...
	if(function_exists('properties_after_insert')){
		...
		$args=array();
		if(!properties_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert ignore into membership_userrecords set tableName='properties', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function properties_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('properties');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='properties' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='properties' and pkValue='$selected_id'");
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `applications_leases` where `property`='".addslashes($id[0])."'", $eo);
	...
	$res = sql("select `id` from `properties` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `units` where `property`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function properties_update($selected_id){
	...
	if(function_exists('properties_before_update')){
		$args=array();
		if(!properties_before_update($data, getMemberInfo(), $args)){ return false; }
	}
...
function properties_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	<script>
		...
		var current_owner__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['owner'] : $filterer_owner); ?>"};
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			if(!$j("#owner-container__RAND__").length){
				$j.ajax({
					...
					$j('[id=owner-container-readonly__RAND__]').html('<span id="owner-match-text">' + resp.results[0].text + '</span>');
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(property_name)%%>', htmlspecialchars($row['property_name'], ENT_QUOTES, 'UTF-8'), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(number_of_units)%%>', htmlspecialchars($row['number_of_units'], ENT_QUOTES, 'UTF-8'), $templateCode);
