	...
	$lookups = array(   
		...
		'residence_and_rental_history' => array(   
			...
			'monthly_rent' => array(
				'parent_table' => 'applicants_and_tenants',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(    CHAR_LENGTH(`units3`.`rental_amount`), CONCAT_WS(\'\',   `units3`.`rental_amount`), \'\')',
				'parent_from' => '`applicants_and_tenants` LEFT JOIN `units` as units1 ON `units1`.`id`=`applicants_and_tenants`.`property` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units1`.`property` LEFT JOIN `units` as units2 ON `units2`.`id`=`applicants_and_tenants`.`unit` LEFT JOIN `properties` as properties2 ON `properties2`.`id`=`units2`.`property` LEFT JOIN `units` as units3 ON `units3`.`id`=`applicants_and_tenants`.`monthly_rent` LEFT JOIN `units` as units4 ON `units4`.`id`=`applicants_and_tenants`.`security_deposit` LEFT JOIN `units` as units5 ON `units5`.`id`=`applicants_and_tenants`.`other_charges` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'security_deposit' => array(
				'parent_table' => 'applicants_and_tenants',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(    CHAR_LENGTH(`units4`.`deposit_amount`), CONCAT_WS(\'\',   `units4`.`deposit_amount`), \'\')',
				'parent_from' => '`applicants_and_tenants` LEFT JOIN `units` as units1 ON `units1`.`id`=`applicants_and_tenants`.`property` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units1`.`property` LEFT JOIN `units` as units2 ON `units2`.`id`=`applicants_and_tenants`.`unit` LEFT JOIN `properties` as properties2 ON `properties2`.`id`=`units2`.`property` LEFT JOIN `units` as units3 ON `units3`.`id`=`applicants_and_tenants`.`monthly_rent` LEFT JOIN `units` as units4 ON `units4`.`id`=`applicants_and_tenants`.`security_deposit` LEFT JOIN `units` as units5 ON `units5`.`id`=`applicants_and_tenants`.`other_charges` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'other_charges' => array(
				'parent_table' => 'applicants_and_tenants',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(    CHAR_LENGTH(`units5`.`other_charges`), CONCAT_WS(\'\',   `units5`.`other_charges`), \'\')',
				'parent_from' => '`applicants_and_tenants` LEFT JOIN `units` as units1 ON `units1`.`id`=`applicants_and_tenants`.`property` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units1`.`property` LEFT JOIN `units` as units2 ON `units2`.`id`=`applicants_and_tenants`.`unit` LEFT JOIN `properties` as properties2 ON `properties2`.`id`=`units2`.`property` LEFT JOIN `units` as units3 ON `units3`.`id`=`applicants_and_tenants`.`monthly_rent` LEFT JOIN `units` as units4 ON `units4`.`id`=`applicants_and_tenants`.`security_deposit` LEFT JOIN `units` as units5 ON `units5`.`id`=`applicants_and_tenants`.`other_charges` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'applicants_and_tenants' => array(   
			'property' => array(
				'parent_table' => 'units',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`units`.`property`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`properties1`.`property_name`), CONCAT_WS(\'\',   `properties1`.`property_name`), \'\'), \' - \'), \'\')',
				'parent_from' => '`units` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units`.`property` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'monthly_rent' => array(
				'parent_table' => 'units',
				'parent_pk_field' => 'id',
				'parent_caption' => '`units`.`rental_amount`',
				'parent_from' => '`units` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units`.`property` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'security_deposit' => array(
				'parent_table' => 'units',
				'parent_pk_field' => 'id',
				'parent_caption' => '`units`.`deposit_amount`',
				'parent_from' => '`units` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units`.`property` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'other_charges' => array(
				'parent_table' => 'units',
				'parent_pk_field' => 'id',
				'parent_caption' => '`units`.`other_charges`',
				'parent_from' => '`units` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`units`.`property` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
