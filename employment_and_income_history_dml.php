...
function employment_and_income_history_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='employment_and_income_history', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function employment_and_income_history_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(employer_name)%%>', htmlspecialchars($row['employer_name'], ENT_QUOTES, 'UTF-8'), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(employer_phone)%%>', htmlspecialchars($row['employer_phone'], ENT_QUOTES, 'UTF-8'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(employer_phone)%%>', urlencode($urow['employer_phone']), $templateCode);
		$templateCode=str_replace('<%%VALUE(employed_from)%%>', @date('m/d/Y', @strtotime(htmlspecialchars($row['employed_from'], ENT_QUOTES, 'UTF-8'))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(employed_from)%%>', urlencode(@date('m/d/Y', @strtotime(htmlspecialchars($urow['employed_from'], ENT_QUOTES, 'UTF-8')))), $templateCode);
		$templateCode=str_replace('<%%VALUE(employed_till)%%>', @date('m/d/Y', @strtotime(htmlspecialchars($row['employed_till'], ENT_QUOTES, 'UTF-8'))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(employed_till)%%>', urlencode(@date('m/d/Y', @strtotime(htmlspecialchars($urow['employed_till'], ENT_QUOTES, 'UTF-8')))), $templateCode);
		$templateCode=str_replace('<%%VALUE(occupation)%%>', htmlspecialchars($row['occupation'], ENT_QUOTES, 'UTF-8'), $templateCode);
