...
function references_insert(){
	...
	$data['reference_name'] = makeSafe($_POST['reference_name']);
	...
	if(function_exists('references_after_insert')){
		$res = sql("select * from `references` where `id`='" . makeSafe($recID) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID);
		$args=array();
		if(!references_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert into membership_userrecords set tableName='references', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function references_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	$combo_tenant->HTML = '<span id="tenant-container' . $rnd1 . '"></span><input type="hidden" name="tenant" id="tenant' . $rnd1 . '" value="' . htmlspecialchars($combo_tenant->SelectedData) . '">';
	$combo_tenant->MatchText = '<span id="tenant-container-readonly' . $rnd1 . '"></span><input type="hidden" name="tenant" id="tenant' . $rnd1 . '" value="' . htmlspecialchars($combo_tenant->SelectedData) . '">';
			...
			jQuery("#tenant-container__RAND__").select2({
				...
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: current_tenant__RAND__.value, t: 'references', f: 'tenant' }
					}).done(function(resp){
						c({
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/references_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/references_templateDV.html');
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return references_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$parent_tables = array(  'applicants_and_tenants' => array('tenant', 'Tenant'));
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(reference_name)%%>', htmlspecialchars($row['reference_name'], ENT_QUOTES), $templateCode);
