...
function references_insert(){
	...
	sql("insert ignore into membership_userrecords set tableName='references', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function references_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(reference_name)%%>', htmlspecialchars($row['reference_name'], ENT_QUOTES, 'UTF-8'), $templateCode);
