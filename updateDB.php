<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('residence_and_rental_history', "create table if not exists `residence_and_rental_history` (   `id` INT unsigned not null auto_increment , primary key (`id`), `tenant` INT unsigned , `monthly_rent` DECIMAL(6,2) , `rent_paid` DECIMAL(6,2) , `rent_balance` DECIMAL(6,2) , `rent_reminder` DATE , `late_rent_reminder` DATE , `duration_of_residency_from` DATE , `to` DATE , `reason_for_leaving` VARCHAR(40) , `notes` TEXT ) CHARSET utf8", $silent);
