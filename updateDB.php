<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('residence_and_rental_history', "create table if not exists `residence_and_rental_history` (   `id` INT unsigned not null auto_increment , primary key (`id`), `tenant` INT unsigned , `month` DATE , `monthly_rent` INT unsigned , `security_deposit` INT unsigned , `other_charges` INT unsigned , `rent_paid` DECIMAL(10,2) , `rent_balance` DECIMAL(10,2) , `rent_reminder` DATE , `due_date` DATE , `late_rent_reminder` DATE , `status` VARCHAR(40) , `duration_of_residency_from` DATE , `to` DATE , `reason_for_leaving` VARCHAR(40) , `notes` TEXT ) CHARSET utf8", $silent);
		setupIndexes('residence_and_rental_history', array('tenant','monthly_rent','security_deposit','other_charges'));
