<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('maintenance_tasks', "create table if not exists `maintenance_tasks` (   `property` INT unsigned , `unit` INT unsigned , `task` INT , `due_date` DATE , `assigned_to` VARCHAR(40) , `task_ID` INT not null auto_increment , primary key (`task_ID`)) CHARSET utf8", $silent);
		setupIndexes('maintenance_tasks', array('property','unit','task'));
		setupTable('employees', "create table if not exists `employees` (   `name` VARCHAR(40) , `address` VARCHAR(40) , `phone` VARCHAR(40) , `cell` VARCHAR(40) , `email` VARCHAR(80) , `employee_ID` INT not null auto_increment , primary key (`employee_ID`)) CHARSET utf8", $silent);
		setupTable('task_types', "create table if not exists `task_types` (   `task_type` VARCHAR(40) , `task_type_ID` INT not null auto_increment , primary key (`task_type_ID`)) CHARSET utf8", $silent);
