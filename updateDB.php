<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('residence_and_rental_history', "create table if not exists `residence_and_rental_history` (   `id` INT unsigned not null auto_increment , primary key (`id`), `tenant` INT unsigned , `month` DATE , `monthly_rent` DECIMAL(10,2) , `security_deposit` DECIMAL(10,2) , `other_charges` DECIMAL(10,2) , `rent_paid` DECIMAL(10,2) , `rent_balance` DECIMAL(10,2) , `rent_reminder` DATE , `due_date` DATE , `late_rent_reminder` DATE , `duration_of_residency_from` DATE , `to` DATE , `reason_for_leaving` VARCHAR(40) , `notes` TEXT ) CHARSET utf8", $silent, array( "ALTER TABLE residence_and_rental_history ADD `field15` VARCHAR(40)","ALTER TABLE `residence_and_rental_history` CHANGE `field15` `month` VARCHAR(40) "," ALTER TABLE `residence_and_rental_history` CHANGE `month` `month` DATE "));
		...
		setupTable('units', "create table if not exists `units` (   `id` INT unsigned not null auto_increment , primary key (`id`), `property` INT unsigned , `unit_number` VARCHAR(40) , `photo` VARCHAR(40) , `status` VARCHAR(40) not null , `features` TEXT , `rental_amount` DECIMAL(6,2) , `deposit_amount` DECIMAL(6,2) , `description` TEXT ) CHARSET utf8", $silent);
