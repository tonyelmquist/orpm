	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			...
			'residence_and_rental_history' => array('Rental history', 'Records of the tenant residence and rental history.', 'resources/table_icons/document_comment_above.png', 'None'),
			...
			'maintenance_tasks' => array('Maintenance tasks', '', 'table.gif', 'None'),
			'employees' => array('Employees', '', 'table.gif', 'None'),
			'task_types' => array('Task types', '', 'table.gif', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'applicants_and_tenants' => "`applicants_and_tenants`.`id` as 'id', `applicants_and_tenants`.`last_name` as 'last_name', `applicants_and_tenants`.`first_name` as 'first_name', `applicants_and_tenants`.`email` as 'email', `applicants_and_tenants`.`phone` as 'phone', if(`applicants_and_tenants`.`birth_date`,date_format(`applicants_and_tenants`.`birth_date`,'%m/%d/%Y'),'') as 'birth_date', `applicants_and_tenants`.`status` as 'status', IF(    CHAR_LENGTH(`properties1`.`property_name`), CONCAT_WS('',   `properties1`.`property_name`), '') as 'property', IF(    CHAR_LENGTH(`properties2`.`property_name`) || CHAR_LENGTH(`units1`.`unit_number`), CONCAT_WS('',   `properties2`.`property_name`, ' - ', `units1`.`unit_number`), '') as 'unit', IF(    CHAR_LENGTH(`units1`.`rental_amount`), CONCAT_WS('',   `units1`.`rental_amount`), '') as 'monthly_rent', IF(    CHAR_LENGTH(`units1`.`deposit_amount`), CONCAT_WS('',   `units1`.`deposit_amount`), '') as 'security_deposit', IF(    CHAR_LENGTH(`units1`.`other_charges`), CONCAT_WS('',   `units1`.`other_charges`), '') as 'other_charges', `applicants_and_tenants`.`current_month_rent_status` as 'current_month_rent_status', `applicants_and_tenants`.`total_rent_due` as 'total_rent_due'",
			'properties' => "`properties`.`id` as 'id', `properties`.`property_name` as 'property_name', `properties`.`type` as 'type', `properties`.`number_of_units` as 'number_of_units', `properties`.`photo` as 'photo', IF(    CHAR_LENGTH(`rental_owners1`.`first_name`) || CHAR_LENGTH(`rental_owners1`.`last_name`), CONCAT_WS('',   `rental_owners1`.`first_name`, ' ', `rental_owners1`.`last_name`), '') as 'owner', `properties`.`country` as 'country', `properties`.`street` as 'street', `properties`.`City` as 'City', `properties`.`State` as 'State', `properties`.`zipCode` as 'zipCode'",
			...
			'maintenance_tasks' => "IF(    CHAR_LENGTH(`properties1`.`property_name`), CONCAT_WS('',   `properties1`.`property_name`), '') as 'property', IF(    CHAR_LENGTH(`units1`.`unit_number`), CONCAT_WS('',   `units1`.`unit_number`), '') as 'unit', IF(    CHAR_LENGTH(`task_types1`.`task_type`), CONCAT_WS('',   `task_types1`.`task_type`), '') as 'task', if(`maintenance_tasks`.`due_date`,date_format(`maintenance_tasks`.`due_date`,'%m/%d/%Y'),'') as 'due_date', `maintenance_tasks`.`assigned_to` as 'assigned_to', `maintenance_tasks`.`task_ID` as 'task_ID'",
			'employees' => "`employees`.`name` as 'name', `employees`.`address` as 'address', `employees`.`phone` as 'phone', `employees`.`cell` as 'cell', `employees`.`email` as 'email', `employees`.`employee_ID` as 'employee_ID'",
			'task_types' => "`task_types`.`task_type` as 'task_type', `task_types`.`task_type_ID` as 'task_type_ID'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'maintenance_tasks' => "`maintenance_tasks` LEFT JOIN `properties` as properties1 ON `properties1`.`id`=`maintenance_tasks`.`property` LEFT JOIN `units` as units1 ON `units1`.`id`=`maintenance_tasks`.`unit` LEFT JOIN `task_types` as task_types1 ON `task_types1`.`task_type_ID`=`maintenance_tasks`.`task` ",
			'employees' => "`employees` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('landlordTenantSystems_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('landlordTenantSystems_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['landlordTenantSystems_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['landlordTenantSystems_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><img src="logoSmall.png" /></a>
			</div>
