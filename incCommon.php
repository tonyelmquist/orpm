	...
	*/
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'applicants_and_tenants' => "`applicants_and_tenants`.`id` as 'id', `applicants_and_tenants`.`last_name` as 'last_name', `applicants_and_tenants`.`first_name` as 'first_name', `applicants_and_tenants`.`email` as 'email', `applicants_and_tenants`.`phone` as 'phone', if(`applicants_and_tenants`.`birth_date`,date_format(`applicants_and_tenants`.`birth_date`,'%m/%d/%Y'),'') as 'birth_date', `applicants_and_tenants`.`status` as 'status', IF(    CHAR_LENGTH(`properties1`.`property_name`), CONCAT_WS('',   `properties1`.`property_name`), '') as 'property', IF(    CHAR_LENGTH(`properties2`.`property_name`) || CHAR_LENGTH(`units2`.`unit_number`), CONCAT_WS('',   `properties2`.`property_name`, ' - ', `units2`.`unit_number`), '') as 'unit', IF(    CHAR_LENGTH(`units3`.`rental_amount`), CONCAT_WS('',   `units3`.`rental_amount`), '') as 'monthly_rent', IF(    CHAR_LENGTH(`units4`.`deposit_amount`), CONCAT_WS('',   `units4`.`deposit_amount`), '') as 'security_deposit', IF(    CHAR_LENGTH(`units5`.`other_charges`), CONCAT_WS('',   `units5`.`other_charges`), '') as 'other_charges', `applicants_and_tenants`.`current_month_rent_status` as 'current_month_rent_status'",
