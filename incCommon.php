	...
	*/
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'residence_and_rental_history' => "`residence_and_rental_history`.`id` as 'id', IF(    CHAR_LENGTH(`applicants_and_tenants1`.`first_name`) || CHAR_LENGTH(`applicants_and_tenants1`.`last_name`), CONCAT_WS('',   `applicants_and_tenants1`.`first_name`, ' ', `applicants_and_tenants1`.`last_name`), '') as 'tenant', CONCAT('Kshs ', FORMAT(`residence_and_rental_history`.`monthly_rent`, 2)) as 'monthly_rent', CONCAT('Kshs ', FORMAT(`residence_and_rental_history`.`security_deposit`, 2)) as 'security_deposit', CONCAT('Kshs ', FORMAT(`residence_and_rental_history`.`rent_paid`, 2)) as 'rent_paid', CONCAT('Kshs ', FORMAT(`residence_and_rental_history`.`rent_balance`, 2)) as 'rent_balance', if(`residence_and_rental_history`.`rent_reminder`,date_format(`residence_and_rental_history`.`rent_reminder`,'%m/%d/%Y'),'') as 'rent_reminder', if(`residence_and_rental_history`.`late_rent_reminder`,date_format(`residence_and_rental_history`.`late_rent_reminder`,'%m/%d/%Y'),'') as 'late_rent_reminder', if(`residence_and_rental_history`.`duration_of_residency_from`,date_format(`residence_and_rental_history`.`duration_of_residency_from`,'%m/%d/%Y'),'') as 'duration_of_residency_from', if(`residence_and_rental_history`.`to`,date_format(`residence_and_rental_history`.`to`,'%m/%d/%Y'),'') as 'to', `residence_and_rental_history`.`reason_for_leaving` as 'reason_for_leaving', `residence_and_rental_history`.`notes` as 'notes'",
